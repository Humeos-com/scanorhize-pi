...
# Define the replacements for external resources
RESOURCE_REPLACEMENTS = [
    {
        "pattern": r'<link rel="stylesheet" href="https://stackpath\.bootstrapcdn\.com/bootstrap/4\.3\.1/css/bootstrap\.min\.css"[^>]*>',
        "replacement": '<link rel="stylesheet" href="{{ url_for(\'static\', filename=\'css/bootstrap.min.css\') }}">'
    },
    {
        "pattern": r'<script src="https://code\.jquery\.com/jquery-3\.3\.1\.slim\.min\.js"[^>]*></script>',
        "replacement": '<script src="{{ url_for(\'static\', filename=\'js/jquery-3.3.1.slim.min.js\') }}"></script>'
    },
    {
        "pattern": r'<script src="https://cdnjs\.cloudflare\.com/ajax/libs/popper\.js/1\.14\.7/umd/popper\.min\.js"[^>]*></script>',
        "replacement": '<script src="{{ url_for(\'static\', filename=\'js/popper.min.js\') }}"></script>'
    },
    {
        "pattern": r'<script src="https://stackpath\.bootstrapcdn\.com/bootstrap/4\.3\.1/js/bootstrap\.min\.js"[^>]*></script>',
        "replacement": '<script src="{{ url_for(\'static\', filename=\'js/bootstrap.min.js\') }}"></script>'
    }
]
