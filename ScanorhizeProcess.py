...
for CurrentScanner in listScannerconfigs:
    ...
    if (CurrentDateinS>ScannerStartinS and CurrentDateinS>=NextStartseconds[i_scan]):
        ...
        if(Scanner.error==0):
            ...
            USB=USBSpace()
            WriteTimeLogfile("Bat :"+str(Bat[1])+"USB :"+str(USB[0])+"Temp :"+str(Temperature))
...
if Bat[1]<0:#si plus de batterie on ne réveille plus le système
    nextStartDateValue=CalculNextStartDate(Scanner.StartDate,(3600*24*30),DateStart)
...
result = run(cmdeject, capture_output=True, universal_newlines=True, shell=True)
